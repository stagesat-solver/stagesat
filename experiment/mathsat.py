...
def main():
    parser = argparse.ArgumentParser(description="Run a benchmark of SMT2 files using mathsat with a time limit.")
    ...
    try:
        with open(args.csv_file, mode='r', newline='', encoding='utf-8') as infile, \
            ...
            for row in reader:
                ...
                command = f"timeout {args.timeout}s /usr/bin/time -f \"real %e\" mathsat \"{filepath}\""
