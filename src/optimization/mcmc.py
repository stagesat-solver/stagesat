...
def mcmc(args, i):
    ...
    for round_num in range(args.nStartOver):
        ...
        sp = np.zeros(foo_square.dim) + args.startPoint + i
        ...
        if R_star < best_R_star:
            best_R_star = R_star
            best_X_star = X_star
            print(f"//////////////////////////////R2 found a best_R_star: {best_R_star}")
        if best_R_star == 0:
            break
        ...
        #     X3_moved = np.array([
        #         nth_fp_dispatchers[j](n_val, x_base_val)
        #         for j, (n_val, x_base_val) in enumerate(zip(N, X_3))
        #     ])
        ...
        # X_star = np.array([
        #             nth_fp_dispatchers[j](n_val, x_base_val)
        #             for j, (n_val, x_base_val) in enumerate(zip(res_round3.x, X_3))])
        if R_star < best_R_star:
            best_R_star = R_star
            best_X_star = X_star
            print(f"??????????????????????????R3 found a best_R_star: {best_R_star}")
        if best_R_star == 0:
            break
